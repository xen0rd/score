<div class="container">
	<?=form_open('user/register', 'id=user_form autocomplete="off"')?>
		<div class="row">
			<div class="col-md-5">
				<fieldset>
			...
			<div class="col-md-5">
				<fieldset>
					...
					<div class="form-group">
						<label for="role_id" class="label label-danger">Role</label>
						<select name="role_id" id="role_id" class="form-control">
							<option disabled selected>Please select</option>
							<?php foreach($roles as $role){ ?>
								<option value="<?=$role->id?>" <?=set_select("role_id", $role->role_name)?>><?=ucfirst($role->role_name)?></option>
							<?php }?>
						</select>
						<?=form_error('role_id');?>
					</div>
					<div class="form-group">
						<label for="department_id" class="label label-danger">Department</label>
						<select name="department_id" id="department_id" class="form-control">
							<option disabled selected>Please select</option>
							<?php foreach($departments as $department){ ?>
							
								<option value="<?=$department->id?>" <?=set_select("department_id", $department->id)?>><?=ucfirst($department->department_name)?></option>
							<?php }?>
						</select>
